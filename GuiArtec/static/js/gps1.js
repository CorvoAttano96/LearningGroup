    ...
    var map = new GMaps({
      ...
      click: function(e){
        ...
        success: function(position) {
          ...
          function(data){
            ...
            for(i=0; i< len ; i++){
                ...
                var marcador = map.addMarker({
                  ...
                  click: function(d){  
                    path = [[tulat, tulng],[tulat,d.position.lng()],[d.position.lat(), d.position.lng()],[d.position.lat(), tulng]];                
                    map.removePolylines();
                    $.getJSON('../senderoEsculturas/esculturasMarkers1',
                    function(data){
          ...
          //Para que cuando el usuario elija uun destino cualquiera para su ruta tambien se filtren los marcadores
          ...
          $.getJSON('../senderoEsculturas/esculturasMarkers1',
                    function(data){
                          ...
                          callback: route => {
                            ...
                            camino = route.legs[0].steps[k].path;
                              for (var c=0;c<camino.length;c++){
                                ...
                                for(i=0; i< data.length ; i++){
                                  var check0 = (map.checkGeofence(data[i].lat,data[i].lng,circle));
                                  if (check0 && data[i].lat != e.latLng.lat() && data[i].lng != e.latLng.lng()) {
                                    marcadores[i].setVisible(true)
                                  }    
                                }
                              }
                        ...
                        for(i=0; i< data.length ; i++){
                          var check1 = (map.checkGeofence(data[i].lat,data[i].lng,circle));
                          if (check1 && data[i].lat != e.latLng.lat() && data[i].lng != e.latLng.lng()) {
                            marcadores[i].setVisible(true)
                          }    
                        }
                        ...
                        for(i=0; i< data.length ; i++){
                          if (data[i].lat == e.latLng.lat() && data[i].lng == e.latLng.lng()) {
                            marcadores[i].setVisible(true)  
                          }; 
                        }
